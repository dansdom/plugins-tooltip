...
(function ($) {
	...
	$.fn.tooltip = function (config) {
		...
		if ($("#" + opts.toolID).length < 1) {
			tooltip = '<div id="' + opts.toolID + '" style="position:absolute;z-index:1000;display:none;"></div>';
			$("body").append(tooltip);
		}
		...
		this.each(function () {
			...
			el.mouseout(function () {
				if (opts.focusEvent && el.onFocus === true) {
					return false;
				}
				el.onEl = false;
	...
	$.fn.tooltip.showTip = function (el, opts) {
		...
		if (opts.staticTool === true) {
			...
			var elOffset = el.offset(),
				...
				toolX = el.tool.outerWidth(),
				toolY = el.tool.outerHeight(),
				elX = el.outerWidth(),
				elY = el.outerHeight();
	...
	$.fn.tooltip.hideTip = function (el) {
		// hide the tool tip
